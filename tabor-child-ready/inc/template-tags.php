	...
	function tabor_accessibility_settings() {
		...
		if ( $accessibility || is_customize_preview() ) { ?>
			...
			<div id="c-settings" class="c-settings <?php echo esc_attr( $accessibility_visibility ); ?>">
				<button id="settings-toggle" class="c-settings__toggle search-toggle button--chromeless" aria-label="<?php esc_attr_e( 'Show/hide accessibility settings', '@@textdomain' ); ?>" aria-controls="settings" aria-expanded="false">
			...
			<div class="c-settings__inner header-font medium smooth h5">
				...
				<div class="c-settings__item">
					<div class="c-settings__item-inner flex items-center">
						<span class="c-settings__label gray"><?php apply_filters( 'tabor_accessibility_settings_night_mode_label', esc_html_e( 'Night Mode', '@@textdomain' ) ); ?></span>
						<button tabindex="0" class="c-settings__switch c-settings__switch--night-mode c-switch" role="switch" aria-checked="false" aria-label="<?php apply_filters( 'tabor_accessibility_settings_night_mode_aria_label', esc_attr_e( 'Toggle Night Mode', '@@textdomain' ) ); ?>"></button>
					</div>
				</div>
				<div class="c-settings__item text-size">
					<div class="c-settings__item-inner flex items-center">
						<span class="c-settings__label gray"><?php apply_filters( 'tabor_accessibility_settings_text_size_label', esc_html_e( 'Text Size', '@@textdomain' ) ); ?></span>
						<button class="c-settings__text-size" tabindex="0" aria-label="<?php apply_filters( 'tabor_accessibility_settings_text_size_label', esc_attr_e( 'Change Text Size', '@@textdomain' ) ); ?>">A</button>
					</div>
				</div>
			</div>
		</div>
...
if ( ! function_exists( 'tabor_customize_more_tag' ) ) :
	/**
	 * Customize preview more tag.
	 *
	 * Generates a live-preview of the more tag within the Customizer.
	 * We're doing this to avoid unnecessary refreshes within the Customizer preview.
	 */
	function tabor_customize_more_tag() {

		global $post;

		if ( is_customize_preview() && strpos( $post->post_content, '<!--more-->' ) ) {

			// Grab the option and check for live-previewing visibility.
			$more_tag            = get_theme_mod( 'more_tag', tabor_defaults( 'more_tag' ) );
			$more_tag_visibility = ( $more_tag ) ? null : 'hidden';

			// Output the custom more tag, if needed.
			echo '<p class="more-link--wrapper ' . esc_attr( $more_tag_visibility ) . '"><a class="button more-link customize-unpreviewable" href=" ' . esc_url( get_permalink() ) . ' "> ' . esc_html( $more_tag ) . ' </a></p>';
		}
	}
endif;
...
if ( ! function_exists( 'tabor_site_info' ) ) :
	...
	function tabor_site_info() {
		...
		if ( $copyrightyear || $themeinfo || $copyrighttext || is_customize_preview() ) {
			...
			echo '<div class="site-info container center-align h5 medium header-font gray" role="contentinfo">';
			...
			if ( $themeinfo || is_customize_preview() ) :
				...
				printf(
					...
					wp_kses( __( '<span class="%1$1s%2$2s"><a href="%3$3s">Powered by %4$4s WordPress Theme</a></span>', '@@textdomain' ), $allowed_html_array ),
					esc_attr( 'site-theme' ),
					esc_attr( $visibility ),
...
if ( ! function_exists( 'tabor_post_media' ) ) :
	...
	function tabor_post_media( $post_id ) {
		...
		if ( has_post_format( 'video' ) && $oembed ) {
			...
			$output = sprintf( '<div class="entry-video bottom-spacer center-align">%1$s</div>', wp_oembed_get( esc_url( $oembed ) ) );
...
if ( ! function_exists( 'tabor_posted_on' ) ) :
	...
	function tabor_posted_on() {
		...
		echo '<div class="entry-meta flex items-center h5 medium header-font gray">' . wp_kses( $password_icon, tabor_svg_allowed_html() ), wp_kses( $sticky_icon, tabor_svg_allowed_html() ), wp_kses( $format_icon, tabor_svg_allowed_html() ) . '<span class="posted-on">' . wp_kses( tabor_time_link(), $allowed_html ) . '</span><span class="byline ' . esc_attr( $author_visibility ) . '"> ' . wp_kses( $byline, $allowed_html ) . '</span></div>';
...
if ( ! function_exists( 'tabor_time_link' ) ) :
	...
	function tabor_time_link() {
		...
		$label = ( 'published' === $date ) ? esc_html__( 'Published', '@@textdomain' ) : esc_html__( 'Updated', '@@textdomain' );
...
if ( ! function_exists( 'tabor_site_logo' ) ) :
	...
	function tabor_site_logo() {
		...
		if ( ! has_custom_logo() || is_customize_preview() ) {
			printf( '<h1 itemscope itemtype="http://schema.org/Organization"><a href="%2$s" rel="home" itemprop="url" class="black">%3$s</a></h1>', esc_attr( $visibility ), esc_url( home_url( '/' ) ), esc_html( get_bloginfo( 'name' ) ) );
		...
		if ( tabor_post_has( 'pings', $post->ID ) || comments_open() && post_type_supports( get_post_type(), 'comments' ) ) :
			...
			<div class="flex justify-start items-center <?php echo esc_attr( $comments_visibility ); ?>">
				...
				<a class="comments-trigger button button--mobile-fullwidth center-align" id="comments-trigger" href="<?php comments_link(); ?>">
					...
					<span class="display-none"><?php esc_html_e( 'Close Comments', '@@textdomain' ); ?></span>
					...
					<span class="display-inline-block">
						...
						if ( get_comments_number() ) {
							...
							esc_html_e( 'Show ', '@@textdomain' );

							$comments_number = get_comments_number();
							if ( '1' === $comments_number ) {
								esc_html_e( '1 Comment', '@@textdomain' );
							} else {
								printf(
									esc_html(
							...
							esc_html_e( 'Leave a Comment', '@@textdomain' );
						}
...
if ( ! function_exists( 'tabor_related_categories' ) ) :
	...
	function tabor_related_categories() {
		...
		$label = apply_filters( 'tabor_related_categories_label', esc_html__( 'Related topics:', '@@textdomain' ) );
