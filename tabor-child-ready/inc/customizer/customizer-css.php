...
function tabor_customizer_font_css() {
	...
	$blog_archivefont    = get_theme_mod( 'blog_archiveheader', '15' );
    $blog_fontsize = get_theme_mod( 'blogpost_fontsize', '12' );

	// Return early if both font selections are set to default.
	if ( 'Default' === $heading_font && 'Default' === $body_font && 'Default' === $blog_archivefont && 'Default' === $blog_fontsize ) {
		return;
	}
	...
	$blog_fontsizecss = '15px';
	$blog_fontsizecss = '#blogpost_fontsize > p {
		font-size: ' . esc_attr( $blog_fontsize ) . 'px !important;
	};';
	...
	if ( 'System Fonts' === $body_font || 'System Serif' === $body_font ) {
		...
		$body_css = '
			...
			blockquote.is-large cite, .is-large.bctt-click-to-tweet cite,
	...
	$blog_archivecss = '15px';
	$blog_archivecss = '#blog_archiveheader {
		font-size: ' . esc_attr( $blog_archivefont ) . 'px !important;
	};';




	return wp_strip_all_tags( $heading_css . $body_css . $blog_archivecss . $blog_fontsizecss );
}
